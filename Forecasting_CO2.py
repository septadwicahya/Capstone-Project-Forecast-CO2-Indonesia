...
def show_page(page):
    if page == '(1) Background':
    ...
    elif page =='(2) Exploration & Visualisation Data':
        st.write("""
        ...
        comparison_df = pd.DataFrame(data=[
            ['Single Exp Smoothing', round((mean_squared_error(test_df, single_exp_test_pred)**0.5*100),2), round((mean_absolute_percentage_error(test_df, single_exp_test_pred)*100),2)],
            ['Double Exp Smoothing', round((mean_squared_error(test_df, double_exp_test_pred)**0.5*100),2), round((mean_absolute_percentage_error(test_df, double_exp_test_pred)*100),2)],
            ['ARIMA', round((mean_squared_error(test_df, ar_test_pred)**0.5*100),2), round((mean_absolute_percentage_error(test_df, ar_test_pred)*100),2)]
            ],
...
pages = ['(1) Background', '(2) Exploration & Visualisation Data', '(3) Modeling Data', 'Forecasting Data']

# Sidebar untuk navigasi halaman
st.sidebar.title('Forecasting CO2 Emissions in Indonesia')
selected_page = st.sidebar.radio('Select Pages', pages)
